<template>
  <h1 class="font-bold text-xl text-gray-700">Customer</h1>
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span
                :class="{
                  ...
                  'bg-gray-100 text-gray-800': customer.type === 'regular',
                  'bg-yellow-100 text-yellow-800': customer.type === 'vip',
                  'bg-blue-100 text-blue-800': customer.type === 'wholesale',
                }"
              >
...
const selectedCustomer = ref<number>(-1);
...
const handleSubmit = async () => {
  ...
  try {
    const payload: Customer = {
      ...
      name: formData.name,
      type: formData.type as "regular" | "vip" | "wholesale",
    };

    if (isEditing.value && selectedCustomer.value !== -1) {
      const customerId = customers.value[selectedCustomer.value].id;
      await axios.put(
    ...
    error.value = err.response?.data?.message || "Error saving customer";
...
const confirmDelete = async () => {
  if (selectedCustomer.value === -1) return;
